...
GraphMatrix generateRandGraph(int V, int density) {
    GraphMatrix graph(V);
...
GraphList generateRandList(int V, int density) {
    GraphList graph(V);
    int max_edges = V * (V - 1) / 2;
    int actual_edges = (density * max_edges) / 100;
...
void DijkstraByMatrixTest(int V, int density, const std::string& file) {
    ...
    auto time = 0;
    for (int i = 0; i < 100; i++) {
        GraphMatrix graph = generateRandGraph(V, density);
...
int main() {
    ...
    // int V = 6;
    //
    // std::cout << "Testing GraphMatrix:" << std::endl;
    // GraphMatrix matrixGraph = generateRandGraph(V,99);
    // matrixGraph.print();
    // matrixGraph.Dijkstra(0);
    //
    // std::cout << "\nTesting GraphList:" << std::endl;
    // GraphList listGraph = generateRandList(V,99);
    //
    // listGraph.print();
